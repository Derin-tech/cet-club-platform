...
export default function AnnouncementBanner() {
  ...
  return (
    <AnimatePresence>
      {isVisible && recentAnnouncements.length > 0 && announcement && (
        <motion.div 
          initial={{ height: 0, opacity: 0 }}
          animate={{ height: 'auto', opacity: 1 }}
          exit={{ height: 0, opacity: 0 }}
          className="bg-accent/10 dark:bg-accent/20 text-navy-secondary dark:text-white px-4 py-3 relative sm:flex sm:items-center sm:justify-between border-b border-accent/20 overflow-hidden"
        >
      <div className="flex items-center gap-3 pr-8">
        ...
        <p className="text-sm font-medium">
          ...
          <button 
            ...
            className="absolute top-1/2 -translate-y-1/2 right-4 sm:static sm:translate-y-0 p-1 hover:bg-accent/20 rounded-pill transition-colors text-navy-secondary/60 dark:text-white/60 hover:text-navy-secondary dark:hover:text-white"
            aria-label="Dismiss"
          >
            <X className="h-4 w-4" />
          </button>
