    ...
    <div className="flex flex-col font-sans relative bg-transparent">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="flex-grow">
          ...
          <section className="py-20 bg-bg-soft/60 backdrop-blur-sm border-b border-border-light/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {latestUpdates.map((item, idx) => (
                  <Link to={`/clubs/${item.clubId}`} key={idx} className="block group">
                    <div className="bg-white/80 backdrop-blur-md rounded-lg p-6 border border-border-light shadow-card hover:shadow-hover hover:border-navy-primary/20 transition-all h-full flex flex-col">
        ...
        <footer className="bg-navy-secondary/95 backdrop-blur-md text-white/80 py-8 border-t border-navy-primary/20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-2">
              <img src="/logo.jpg" alt="CET Logo" className="w-6 h-6 rounded-sm object-cover" />
